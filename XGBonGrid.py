...
for file in FILES:
    ...
    for d in depth:
        for w in child_weight:
            ...
            clf = xgb.XGBClassifier(max_depth=d,min_child_weight=w)
            f.write(str(d)+', '+str(w)+', '+str(cross_val_score(clf,np.array(X),y,cv=10,scoring='accuracy',n_jobs=10).mean())+', '+str(timeit.default_timer()-start)+'\n')
    f.close()